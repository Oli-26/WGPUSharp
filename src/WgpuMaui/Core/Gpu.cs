...
using WgpuMaui.Interop;

namespace WgpuMaui.Core;

/// <summary>
/// Entry point for WgpuMaui. Use this to initialize WebGPU and start render loops.
/// </summary>
public static class Gpu
{
    ...
    /// <summary>
    /// Requests a GPU adapter from the browser. This is the first call to make when using WgpuMaui.
    /// Requires the WgpuMaui.js script to be loaded in index.html.
    /// </summary>
    ...
    /// <exception cref="InvalidOperationException">Thrown when the WgpuMaui.js bridge script is not loaded.</exception>
    ...
    public static async Task<GpuAdapter> RequestAdapterAsync(IJSRuntime js, bool preferHighPerformance = false, CancellationToken ct = default)
    {
        ...
        catch (JSException ex) when (ex.Message.Contains("WgpuMaui"))
        {
            throw new InvalidOperationException(
                "WgpuMaui JS bridge not found. Add this to your index.html before the Blazor script:\n" +
