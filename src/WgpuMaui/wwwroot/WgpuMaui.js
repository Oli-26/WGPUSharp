...
window.WgpuMaui = {
    ...
    async getAdapterInfo(adapterId) {
        ...
        try {
            ...
            console.warn('[WgpuMaui] getAdapterInfo failed:', e);
    ...
    registerDeviceLostCallback(deviceId, dotNetRef) {
        ...
        device.lost.then(info => {
            console.error("WgpuMaui: GPU device lost —", info.message, "(reason:", info.reason + ")");
            try {
    ...
    startLoop(dotNetRef) {
        ...
        async function tick(now) {
            ...
            try {
                ...
                if (loopRunning) console.error("WgpuMaui frame error:", e);
            }
